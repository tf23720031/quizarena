...
function readLegacyWrongBook() {
  const keys = ["quizarena_wrong_book", "wrong_question_book"];
  for (const key of keys) {
    try {
      const parsed = JSON.parse(localStorage.getItem(key) || "null");
      if (Array.isArray(parsed) && parsed.length) return { key, items: parsed };
      if (parsed && Array.isArray(parsed.questions) && parsed.questions.length) {
        return { key, items: parsed.questions };
      }
    } catch (_) {}
  }
  return null;
...
async function syncLegacyWrongBookToBackend() {
  const username = getCurrentUser();
  if (!username) return;
  const legacy = readLegacyWrongBook();
  if (!legacy?.items?.length) return;
  try {
    await api("/sync_wrong_book", {
      method: "POST",
      body: JSON.stringify({ username, questions: legacy.items }),
    });
    localStorage.removeItem(legacy.key);
  } catch (error) {
    console.warn("[wrong-book sync] failed:", error.message);
  }
}
...
function renderDailyMissionScene(profile = {}) {
  ...
  const dailyMission = profile?.dailyMission || state.dailyMission || { streak: 0, missions: [] };
  state.dailyMission = dailyMission;
  dailyMissionStreak.textContent = `Active ${Number(dailyMission.streak || 0)} days`;
  const missions = Array.isArray(dailyMission.missions) ? dailyMission.missions : [];
  dailyMissionList.innerHTML = missions.length ? missions.map((mission, index) => {
    const question = mission.question || {};
    const answered = !!mission.answered;
    const tone = mission.status === "correct" ? "blue" : mission.status === "wrong" ? "violet" : "pink";
    const percent = answered ? 100 : 0;
    const optionsHtml = Array.isArray(question.options) && question.options.length
      ? `<div class="daily-mission-options">${
          question.options.map((option, optionIndex) => `
            <button type="button" class="daily-option-btn" data-mission-index="${index}" data-option-index="${optionIndex}" ${answered ? "disabled" : ""}>
      ...
      : "";
    return `
      <article class="daily-mission-card tone-${tone}">
        <div class="daily-mission-top">
          <div>
            <h4>${escapeHtml(mission.title || "Daily Mission")}</h4>
            <p>${answered ? `Status: ${escapeHtml(mission.status)}` : "Answer once to complete today's mission"}</p>
          </div>
          <div class="daily-reward-badge">${escapeHtml(mission.reward || "Reward")}</div>
        </div>
        <div class="daily-mission-question">
          <strong>${escapeHtml(question.title || "Mission Question")}</strong>
          <p>${escapeHtml(question.content || "")}</p>
        </div>
        ${optionsHtml}
        <div class="daily-track"><span style="width:${percent}%"></span></div>
        <button type="button" class="daily-claim-btn ${answered ? "ready" : ""}" disabled>${answered ? mission.status.toUpperCase() : "Waiting Answer"}</button>
      </article>
    `;
  }).join("") : '<div class="profile-wrong-empty">No daily mission is available right now.</div>';
  const missionTotal = Math.max(missions.length, 1);
  const missionDone = missions.filter((mission) => mission.answered).length;
  ...
  dailyMissionSummary.innerHTML = `
    ...
    <div class="profile-helper-text">${escapeHtml(dailyMission.lastCompleteDate ? `Last clear: ${dailyMission.lastCompleteDate}` : "One attempt per mission per day. Wrong answers still consume the mission.")}</div>
  `;
  dailyMissionList.querySelectorAll(".daily-option-btn").forEach((btn) => {
    btn.addEventListener("click", () => submitDailyMissionAnswer(Number(btn.dataset.missionIndex), Number(btn.dataset.optionIndex)));
  });
}

async function submitDailyMissionAnswer(missionIndex, optionIndex) {
  const username = getCurrentUser();
  const missions = Array.isArray(state.dailyMission?.missions) ? state.dailyMission.missions : [];
  const mission = missions[missionIndex];
  if (!username || !mission || mission.answered) return;
  try {
    const data = await api("/daily_mission_answer", {
      ...
      body: JSON.stringify({
        ...
        missionDate: state.dailyMission?.date || "",
        selected: [optionIndex],
      }),
    });
    state.dailyMission = data.dailyMission || state.dailyMission;
    if (state.profile) state.profile.dailyMission = state.dailyMission;
    renderDailyMissionScene(state.profile || {});
    showToast(data.message || (data.status === "correct" ? "Correct!" : "Wrong answer recorded."));
...
function renderProfileSummary(profile, keepLocalState = false) {
  ...
  state.dailyMission = profile?.dailyMission || state.dailyMission || null;
...
async function loadProfileSummary() {
  ...
  try {
    await syncLegacyWrongBookToBackend();

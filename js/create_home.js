...
// ── Legacy localStorage 錯題同步到後端 ────────────────────────────
// 找出舊版 key（quizarena_wrong_book / wrong_question_book），
// 一次寫進後端 wrong_question_book table，然後清掉本地 key。
...
async function syncLegacyWrongBookToBackend() {
  ...
  if (!legacy || !legacy.items || !legacy.items.length) return;
  try {
    await api('/sync_wrong_book', {
      method: 'POST',
      body: JSON.stringify({ username: state.currentUser, questions: legacy.items })
    });
...
(async function init() {
  ...
  // Fire-and-forget: 把舊版 localStorage 錯題同步到後端
  syncLegacyWrongBookToBackend().catch(() => {});

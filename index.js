...
function renderFriendsOverview(data) {
  ...
  if (!records.length || !data.hasWins) {
    ...
    friendsRecords.innerHTML = records.map((item, index) => `
      <article class="friends-record-card">
        <div class="friends-record-top">
          <div>
            <div class="friends-record-rank">${index === 0 ? "MVP" : `RANK ${index + 1}`}</div>
            <div class="friends-record-name">${item.username}</div>
          </div>
          <div class="friends-record-wins">${item.wins} 勝</div>
        </div>
      </article>
    `).join("");
  }

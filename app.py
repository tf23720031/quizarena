...
def get_user_profile(username):
    ...
    with closing(sqlite3.connect(USERS_DB_PATH)) as conn:
        conn.row_factory = dict_factory
        row = conn.execute('''
            SELECT username, avatar_image, selected_title, face, hair, eyes, eyes_offset_y, updated_at
            FROM user_profiles
            WHERE username=?
            LIMIT 1
        ''', (username,)).fetchone()
...
def save_user_profile(username, payload):
    if not username:
        return
    avatar_image = str(payload.get('avatarImage', '') or '').strip()
    ...
    with closing(sqlite3.connect(USERS_DB_PATH)) as conn:
        ...
        ''', (username, avatar_image, selected_title, face, hair, eyes, eyes_offset_y, now_ts()))
        conn.commit()
